...
export async function contexto(empresaId: string, vendedorId: string) {
...
export async function candidatosAgenda(empresaId: string, vendedorId: string, clienteIds: string[] = []): Promise<Candidato[]> {
  const vendedor = await prisma.vendedor.findFirst({ where: { id: vendedorId, empresaId } });
  if (!vendedor?.ativo) throw new AppError(400, 'Vendedor inválido.');
  const clientes = await prisma.cliente.findMany({ where: { empresaId, vendedorId, filialCodigo: vendedor.filialCodigo, ...(clienteIds.length ? { id: { in: clienteIds } } : { oportunidades: { some: { status: { in: ['NOVA', 'EM_ANALISE', 'ENCAMINHADA'] } } } }) }, include: { oportunidades: { where: { empresaId, status: { in: ['NOVA', 'EM_ANALISE', 'ENCAMINHADA'] } }, orderBy: { pontuacao: 'desc' } } } });
...
export async function reservas(db: Prisma.TransactionClient, empresaId: string, destino: string, vendedorId: string, remotos: Ocupacao[], ignorar?: string, ignorarExterno?: number) {
  const planos = await db.planoAgenda.findMany({ where: { empresaId, destino, vendedorId, estado: { in: ['ENVIANDO', 'CONCLUIDO', 'INCERTO'] }, ...(ignorar ? { id: { not: ignorar } } : {}) } });
  const todos = [...remotos];
  ...
  const acoes = await db.acaoAgenda.findMany({ where: { empresaId, destino, vendedorId, estado: { in: ['ENVIANDO', 'CONCLUIDO', 'INCERTO'] } } });
...
export async function previaAgenda(usuarioId: string, empresaId: string, body: unknown) {
  ...
  const ctx = await contexto(empresaId, regras.vendedorId);
  const candidatos = await candidatosAgenda(empresaId, regras.vendedorId, regras.clienteIds);
  const remotos = await ocupacaoRemota(ctx, regras);
  const ocupados = await reservas(prisma, empresaId, ctx.destino, regras.vendedorId, remotos);
  ...
  return prisma.planoAgenda.create({ data: { empresaId, usuarioId, destino: ctx.destino, vendedorId: regras.vendedorId, dados: json(dados) } });
}
export async function confirmarAgenda(usuarioId: string, empresaId: string, id: string) {
  const plano = await prisma.planoAgenda.findFirst({ where: { id, usuarioId, empresaId } });
  ...
  const ctx = await contexto(empresaId, plano.vendedorId);
  ...
  await candidatosAgenda(empresaId, plano.vendedorId, dados.itens.map(i => i.id));
  ...
  const iniciou = await prisma.$transaction(async tx => {
    ...
    const ocupados = await reservas(tx, empresaId, ctx.destino, plano.vendedorId, remotos, id);
    for (const item of dados.itens) {
