...
vi.mock('../src/services/agenda.js', () => ({ contexto: async (_user: string, _empresa: string, vendedorId: string) => ({ destino: 'http://sirius|1', config: { urlBase: 'http://sirius', filialCodigo: 1 }, sessao: { token: 'TESTE' }, vendedor: { id: vendedorId, codigoExterno: 7, nome: 'Vendedor' } }), reservas: mock.reservas }));
...
describe('operações de agenda no Sírius', () => {
  it('consulta geral identifica cada vendedor e relata falhas parciais', async () => {
    ...
    const r = await visualizarAgenda('e1', { inicio: '2030-01-07', fim: '2030-01-07' });
  ...
  it('filtro individual restringe consulta e bloqueia vendedor de outra filial', async () => {
    await visualizarAgenda('e1', { inicio: '2030-01-07', fim: '2030-01-07', vendedorId: 'v1' });
    expect(mock.db.vendedor.findMany).toHaveBeenCalledWith(expect.objectContaining({ where: { empresaId: 'e1', filialCodigo: 1, id: 'v1' } }));
    mock.db.vendedor.findMany.mockResolvedValue([]);
    await expect(visualizarAgenda('e1', { inicio: '2030-01-07', fim: '2030-01-07', vendedorId: 'estranho' })).rejects.toThrow('filial');
  });
  it('não escreve quando o evento mudou ou não pertence ao vendedor', async () => {
    await expect(executarAcaoAgenda('u1', 'e1', 'ALTERAR', { ...edit, versao: '0'.repeat(64) })).rejects.toThrow('mudou');
    await expect(executarAcaoAgenda('u1', 'e1', 'EXCLUIR', { ...base, codigo: 99 })).rejects.toThrow('não encontrado');
  ...
  it('edita, verifica o resultado e libera a reserva antiga preservando o histórico', async () => {
    ...
    expect((await executarAcaoAgenda('u1', 'e1', 'ALTERAR', edit)).estado).toBe('CONCLUIDO');
    expect(mock.db.planoAgenda.update).toHaveBeenCalledWith({ where: { id: 'plano' }, data: { dados: { itens: [{ codigoExterno: 123 }], liberados: [123] } } });
    await executarAcaoAgenda('u1', 'e1', 'ALTERAR', edit); expect(mock.alterar).toHaveBeenCalledTimes(1);
  });
  it('exclui somente o código validado e confirma ausência', async () => {
    ...
    expect((await executarAcaoAgenda('u1', 'e1', 'EXCLUIR', base)).estado).toBe('CONCLUIDO');
    expect(mock.excluir).toHaveBeenCalledWith(expect.anything(), 'TESTE', 12);
    await executarAcaoAgenda('u1', 'e1', 'EXCLUIR', base); expect(mock.excluir).toHaveBeenCalledTimes(1);
  });
  it('não anuncia exclusão se o registro continuar presente', async () => {
    expect((await executarAcaoAgenda('u1', 'e1', 'EXCLUIR', base)).estado).toBe('INCERTO');
  ...
  it('permite atualizar apenas contato de compromisso passado sem criar nova reserva', async () => {
    ...
    expect((await executarAcaoAgenda('u1', 'e1', 'ALTERAR', input)).estado).toBe('CONCLUIDO');
  });
  it('timeout mantém operação incerta e impede repetição', async () => {
    mock.alterar.mockRejectedValue(new Error('timeout'));
    expect((await executarAcaoAgenda('u1', 'e1', 'ALTERAR', edit)).estado).toBe('INCERTO');
    await executarAcaoAgenda('u1', 'e1', 'ALTERAR', edit); expect(mock.alterar).toHaveBeenCalledTimes(1);
  });
  it('criação manual valida cliente, reserva antes do POST e verifica leitura posterior', async () => {
    ...
    expect((await executarAcaoAgenda('u1', 'e1', 'INCLUIR', input)).estado).toBe('CONCLUIDO');
    await executarAcaoAgenda('u1', 'e1', 'INCLUIR', input); expect(mock.incluir).toHaveBeenCalledTimes(1);
    await expect(executarAcaoAgenda('u2', 'e1', 'INCLUIR', input)).rejects.toThrow('já foi utilizada');
  });
  it('não envia inclusão se o cliente não pertence à carteira', async () => {
    mock.db.cliente.findFirst.mockResolvedValue(null);
    await expect(executarAcaoAgenda('u1', 'e1', 'INCLUIR', { chave: base.chave, vendedorId: 'v1', clienteId: 'estranho', dia: '2030-01-08', inicio: '10:00', fim: '10:30', tipoInteracao: 'R' })).rejects.toThrow('carteira');
  ...
  it('bloqueia edição de evento com resultado de operação anterior ainda incerto', async () => {
    ...
    await expect(executarAcaoAgenda('u1', 'e1', 'ALTERAR', edit)).rejects.toThrow('pendente');
