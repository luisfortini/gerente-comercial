...
vi.mock('../src/services/agenda.js', () => ({ contexto: async (_u: string, _empresa: string, vendedorId: string) => ({ destino: 'http://sirius|1', config: { urlBase: 'http://sirius', filialCodigo: 1, loginSirius: 'operador.teste' }, sessao: { token: 'TOKEN' }, vendedor: { id: vendedorId, codigoExterno: 7 } }) }));
...
describe('operação CRM da agenda', () => {
  it('carrega e ordena a tabela de ocorrências do Sírius', async () => {
    const r = await ocorrenciasCrm('e1', 'v1');
  ...
  it('rejeita uma tabela de ocorrências malformada', async () => {
    ...
    await expect(ocorrenciasCrm('e1', 'v1')).rejects.toThrow('formato não reconhecido');
  });
  it('valida agenda e ocorrência, persiste antes do POST e usa o login configurado', async () => {
    ...
    expect((await registrarAtendimentoCrm('u1', 'e1', 12, input)).estado).toBe('CONCLUIDO');
  });
  it('não repete o envio com a mesma chave e os mesmos dados', async () => {
    await registrarAtendimentoCrm('u1', 'e1', 12, input); await registrarAtendimentoCrm('u1', 'e1', 12, input);
    expect(mock.inserir).toHaveBeenCalledTimes(1);
  });
  it('recusa reutilizar a chave com outro conteúdo', async () => {
    await registrarAtendimentoCrm('u1', 'e1', 12, input);
    await expect(registrarAtendimentoCrm('u1', 'e1', 12, { ...input, observacao: 'Conteúdo diferente' })).rejects.toThrow('outros dados');
  });
  it('impede segundo atendimento para o mesmo compromisso', async () => {
    ...
    await expect(registrarAtendimentoCrm('u1', 'e1', 12, { ...input, chave: '7e111c4d-96cd-4ac1-a1ca-2355ef4dfd56' })).rejects.toThrow('já possui');
    expect(mock.inserir).not.toHaveBeenCalled();
  });
  it('recusa ocorrência inexistente e agenda alterada', async () => {
    await expect(registrarAtendimentoCrm('u1', 'e1', 12, { ...input, codigoOcorrencia: 999 })).rejects.toThrow('ocorrência válida');
    await expect(registrarAtendimentoCrm('u1', 'e1', 12, { ...input, versao: '0'.repeat(64) })).rejects.toThrow('mudou');
    expect(mock.inserir).not.toHaveBeenCalled();
  });
  it('recusa tipo de interação fora de P e R', async () => {
    await expect(registrarAtendimentoCrm('u1', 'e1', 12, { ...input, tipoInteracao: 'TELEFONE' })).rejects.toThrow();
    expect(mock.inserir).not.toHaveBeenCalled();
  });
  it('marca timeout como incerto e não reenvia', async () => {
    mock.inserir.mockRejectedValue(new Error('timeout'));
    expect((await registrarAtendimentoCrm('u1', 'e1', 12, input)).estado).toBe('INCERTO');
    await registrarAtendimentoCrm('u1', 'e1', 12, input); expect(mock.inserir).toHaveBeenCalledTimes(1);
  });
  it('lista histórico somente no destino, usuário e intervalo solicitados', async () => {
    await listarAtendimentosCrm('e1', '2030-01-01', '2030-01-31', 'v1');
    expect(mock.db.atendimentoAgenda.findMany).toHaveBeenCalledWith(expect.objectContaining({ where: expect.objectContaining({ empresaId: 'e1', destino: 'http://sirius|1', vendedorId: 'v1' }) }));
    await expect(listarAtendimentosCrm('e1', '2030-02-01', '2030-01-01')).rejects.toThrow('Período inválido');
  });
});
